...
import 'package:gemailai/widgets/processing_screen.dart';
import 'package:gemailai/widgets/text_field.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';

import 'code_feature_test_ui.dart';
...
class CodeGenerationUi extends StatefulWidget {
  const CodeGenerationUi({super.key});

  @override
  State<CodeGenerationUi> createState() => _CodeGenerationUiState();
}

class _CodeGenerationUiState extends State<CodeGenerationUi> {
  TextEditingController tcForAppName = TextEditingController();
  final FocusNode _focusNode = FocusNode();
  String app_name = "", language = "";
  String _result = "";
  ...
  Future<void> _pickPdf() async {
    ...
    if (result != null) {
      ...
      t = Timer(const Duration(seconds: 3), () {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (builder) => FeaturesWidget(features: features, app_name: "messenger")));
      });
  ...
  void initState() {
    ...
    tcForAppName.addListener(() {
      setState(() {
        app_name = tcForAppName.text;
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    List<String> items = ["c", "cpp", "python", "java"];
    var size = MediaQuery.of(context).size,
        appBarHeight = AppBar().preferredSize.height;
    const List<Job> _list = [
      Job('C', Icons.developer_mode),
      Job('C++', Icons.design_services),
      Job('Python', Icons.account_balance),
      Job('Java', Icons.school),
    ];

    // Future<void> _getCode() async {
    //   String url = 'http://192.168.0.236:3000/get_code?app_name=i want a $app_name app with a modern looking ui&q=Java';
    //
    //   try {
    ...
    //       // },
    //       // body: jsonEncode({
    //       //   "app_name": "i want a $app_name app with beautiful UI",
    //       //   "q": language,
    //       // }),
    //     );
    //
    ...
    //       setState(() {
    //         _result = json.decode(response.body)['code'] ?? "No code generated.";
    //       });
    //       redirect();
    //     } else {
    //       setState(() {
    //         _result = "Failed to generate code: ${response.statusCode}";
    //         processing = false;
    //       });
    //       print(_result);
    //
    ...
    Future<void> _getCode() async {
      // Use Uri to safely encode URL components
      String appName = Uri.encodeQueryComponent(
          'i want a $app_name app with a modern looking ui');
      String language = Uri.encodeQueryComponent('Java');
      String url =
          'http://192.168.0.236:3000/get_code?app_name=$appName&q=$language';
    ...
    Future<void> fetchCode(String appName, String language) async {
      log("calling the API");
      final uri = Uri.parse(
          'http://192.168.0.236:5000/get_code?app_name=$appName&q=$language');
      try {
        final response =
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Visibility(visible: processing, child: ProcessingScreen()),
          Visibility(
            visible: !processing,
            child: SafeArea(
              child: Padding(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      Padding(
                        padding: EdgeInsets.symmetric(vertical: appBarHeight),
                        child: Text.rich(
                            ...
                            TextSpan(children: [
                              ...
                              WidgetSpan(
                                  child: app_name.isEmpty
                                      ? Padding(
                                          padding: const EdgeInsets.symmetric(
                                              horizontal: 8.0),
                                          child: SizedBox(
                                              width: size.width * .041,
                                              height: size.width * .041,
                                              child: Visibility(
                                                visible: app_name.isEmpty,
                                                child:
                              ...
                              TextSpan(
                                  text:
                                      " app using the following language/framework: ",
                                  style: TextStyle(
                                    fontFamily: "SF-Pro",
                                    fontWeight: FontWeight.bold,
                                    color: Colors.white,
                      ...
                      Padding(
                        ...
                        child: CustomTextField(
                          ...
                          hint: "app name",
                          lines: 1,
                          textEditingController: tcForAppName,
                        ),
                      ),
                      Padding(
                        ...
                        child: ClipRRect(
                          ...
                          child: CustomDropdown<Job>.search(
                            hintText: 'Select language/framework',
                            items: _list,
                      ...
                      Padding(
                        ...
                        child: Visibility(
                          visible: app_name.isNotEmpty && language.isNotEmpty,
                          child: Center(
                          ...
                          child: Container(
                            ...
                            child: const Center(
                              child: Text.rich(
                                TextSpan(
                                  children: [
                                    WidgetSpan(child: Icon(Icons.upload_file, color: Colors.purple,),
                                    alignment: PlaceholderAlignment.middle),
                                    TextSpan(
                                      text: "  upload PDF with app details",
                                        style: TextStyle(
                                            height: 0,
                                            fontFamily: "SF-Pro",
                                            fontWeight: FontWeight.bold,
  ...
  void redirect() {
    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
            builder: (builder) => FeaturesWidget(
              app_name: app_name,
                  features: decodeFeatures(_result),
                )));
  }
}
